...
void waitnigTimeToBeFound(proc pro[], int number, int wait[]) 
{  
    wait[0] = 0; //first procs 
    for (int i = 1; i < number ; i++ ) 
        wait[i] = pro[i-1].btt + wait[i-1] ; 
} 
void turnAroundtimeToBFound(proc pro[], int number, int wait[], int tt[]) 
{ 
...
{ 
    ...
    cout << "Average waiting time = "<< (float)total_wait / (float)number; 
